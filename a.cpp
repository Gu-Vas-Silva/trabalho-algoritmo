...
int main()
{
    ...
    Cidade cid[100];
    cid[0] = {1, "Candido-Mota", "sp"};
    Raca raca[100];
    raca[0] = {1, "rato"};
    Tutor tutor[100];
    tutor[0] = {1, "Gustavo", "435.284.888-32", "aaa", 1};
    Animal animal[100];
    animal[0] = {1, "toto", 1, 12, 10.2, 1};
    Veterinario vet[100];
    vet[0] = {1, "Duzao", "Rua A", 1};
    Consulta consul[100];
    consul[0].codigo = 1;
    consul[0].codigo_animal = 1;
    consul[0].codigo_veterinario = 1;
    consul[0].data = "10/10/2010";
    consul[0].dataFormatada = converterStringParaData(consul[0].data); 
    consul[0].valor = 120.00;
    int c;
    c = 0;
    while (c != 6)
    {
...
void realizarConsulta(Consulta a[], Animal animal[], Tutor tutor[], Raca raca[], Veterinario vet[], Cidade cid[], int &contA, int &contRaca, int &contAnimal, int &contVet, int &contCid)
{
    ...
    for (int saida = 1; i < 100 && saida != 0; i++)
    {
        ...
        buscarAnimal(a, animal, tutor, raca, i, contAnimal, contRaca);
        ...
        data= a[i].data;
        a[i].dataFormatada = converterStringParaData(data);
        cout << "\nvalor da consulta: ";
        cin >> a[i].valor;
        cout << "\ndigite 0 para parar";
        cin >> saida;
...
void intervaloData(Consulta a[], Animal animal[], Veterinario vet[], int &contA)
{
    ...
    getline(cin, dataInicialS);
    dataInicial = converterStringParaData(dataInicialS);
    cin.ignore();
    cout << "\ndigite a data final: ";
...
void intervaloDataVet(Consulta a[], Animal animal[], Veterinario vet[], int &contVet, int &contA)
{
    ...
    cin >> codigo;
    buscarVetIntervalo(vet, contVet, &codigo);
    cin.ignore();
    cout << "\ndigite a data inicial: ";
    getline(cin, dataInicialS);
    dataInicial = converterStringParaData(dataInicialS);
...
void buscarVetIntervalo(Veterinario a[], int &contVet, int *codigo)
{
    ...
    while (achou == 0)
    {
        for (int i = 0; i < contVet; i++)
        {
            ...
            {
                cout << "\nVeterinario:" << a[i].nome;
                achou++;
            }
        }
        if (achou == 0)
        {
            cout << "codigo nao encontrado. Digite novamente: ";
            cin >> *codigo;
        }
    }
}

void mostrarConsultaPorIntervalo(Consulta a[], Animal animal[], Veterinario vet[], time_t &dataInicial, time_t &dataFinal, int &contA)
{
    ...
    {
        if (a[i].dataFormatada >= inicio && a[i].dataFormatada <= final)
        {
...
void mostrarConsultaPorIntervaloVet(Consulta a[], Animal animal[], Veterinario vet[], time_t &dataInicial, time_t &dataFinal, int &codigo, int &contA, int &contVet)
{
    ...
    for (i = 0; i <= contVet; i++)
    {
        if (vet[i].codigo == codigo)
        {
            ...
            for (j = 0; j < contA; j++)
            { 
            if(a[j].codigo_veterinario == codigo){
            if (a[j].dataFormatada >= inicio && a[j].dataFormatada <= final)
                {
...
time_t converterStringParaData(string &data)
{
    ...
    while(achou == 0){
        ...
        if(mes < 1 || mes > 12){
            cout<<"\nMes nao existente. Digite a data novamente: ";
            getline(cin, data);
        }else if(dia < 1 || dia > 31){
            cout<<"\nDia nao existente. Digite a data novamente: ";
            getline(cin, data);
        }else if(ano<2000){
            cout<<"\nO ano tem que ser depois de 2000. Digite a data novamente: ";
            getline(cin, data);
        }else{
        achou++;
    }
